...
const deleteTask = function(request, response, next) {
  const [, deletionId] = request.url.match(/.*delete_(\d+)/);
  deleteTaskFromList(deletionId);
  next();
};

const addNewItem = function(request, response, next) {
  appendToItemsList(request.body);
  next();
};
...
const initiateApp = function() {
  ...
  app.delete('/delete_\\d+', deleteTask);
  app.delete('/delete_\\d+', serveTasksList);
